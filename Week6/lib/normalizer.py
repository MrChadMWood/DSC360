...
class Normalizer:
    ...
    def lemmatize_text(self):
        ...
        # Uses %delim% to track rows within series. Keeps delim off last row.
        text = ' '.join(self.iloc[:-1] + '% delim %')
        text += self.iloc[-1]
        ...
        if text[-1:] != ' ':
            ...
            # Corrects issue resulting in words joining without whitespace
            # Alt: if len(nlp_list[-1].split()) < 3:
            # Implications on large text bodies not tested...
            if ' ' not in [nlp_list[-2][-1], nlp_list[-1][0]]:
                nlp_list[-1] = ' ' + nlp_list[-1]

        root_list = ''.join(nlp_list).split('% delim %')
        return pd.Series(root_list)

    # Removes the stopwords from each row
    def remove_stopwords(self, stopword_list=None, is_lower_case=False):
    ...
    def normalize(self, strip_html=False, remove_accented_chars=False,
                  expand_contractions=False, remove_special_chars=False,
                  remove_repeat_chars=False, lemmatize_text=False,
                  remove_stopwords=False, remove_digits=False,
                  text_lower=False, stopwords=None):

        if text_lower:
            self.series = self.series.apply(lambda x: x.lower())
        if strip_html:
        ...
        if remove_stopwords:
            self.series = Normalizer.remove_stopwords(self.series, stopword_list=stopwords)
